...
- **Backlog.** Open work is tracked in `BACKLOG.md`.
- **Agent.** `scripts/agent.mjs` checks postings from allowed sources, queues them for human review and drafts reports; skills for Copilot Studio in `docs/agent-openapi.yaml`, design in `docs/agent.md`.
...
- **Report wrong vibes.** An anonymous report form with no identity fields. Personal details are removed from the story in the browser, a recruiter photo becomes a perceptual hash and is never uploaded, and the reporter previews the exact record before sending.
