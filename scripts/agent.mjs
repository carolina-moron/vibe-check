...
export async function run({ sources, submissions, fetchFn = fetch, dry = false, now = new Date(), minScore = 20 }) {
  ...
  for (const item of items) {
    ...
    await (await getStore()).event({ type: "check", source: String(item.from).startsWith("ats:") ? "agent" : "submission", kind: "job", tier: rec.tier, flags: rec.flags.length, signs: rec.flags.map((f) => f.id), at: now.toISOString() });
